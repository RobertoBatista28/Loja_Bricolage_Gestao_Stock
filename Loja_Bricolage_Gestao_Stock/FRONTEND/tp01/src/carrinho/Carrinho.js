...
const ProductCard = ({ nome, preco, imagem, quantidade, onRemove }) => {
  ...
  return (
    <div className="cart-card">
      ...
      <p className="cart-quantity"><strong>{quantidade} x</strong></p>
...
const CartPage = () => {
  ...
  const removeProduct = async (index) => {
    ...
    if (!productId || !nrVenda) {
      console.error('Produto ou nrVenda nao foram encontrados:', { productId, nrVenda });
      return;
    }
  ...
  return (
    <div className="cart-page">
      ...
      <div className="total-container">
        <div>
          <div className="total-label">TOTAL DA COMPRA:</div>
          <div className="total-label-price">{calcularTotal()} €</div>
        </div>
        <button className="continue-button" onClick={handleFinalizar}>Finalizar compra</button>
      </div>
    </div>
  );
};
