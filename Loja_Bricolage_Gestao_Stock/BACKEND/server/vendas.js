...
const vendasRouter = () => {
    ...
    router.route("/vendas/:nrVenda")
        .get(Utilizadores.authorize([scopes["administrador"], scopes["gestor"], scopes["utilizador"]]), verifyToken, function (req, res, next) {
    ...
    router.route("/venda/me")
        ...
        .post(Utilizadores.authorize([scopes["administrador"], scopes["utilizador"]]), verifyToken, async function (req, res, next) {
            try {
                ...
                if (!produtoStock) {
                    return res.status(400).json({ message: `Não existe stock para a referência do produto ${vendaRequest.referencia}.` });
                }

                // Verificar se há stock suficiente
                if (produtoStock.quantidade < vendaRequest.quantity) {
                    return res.status(400).json({ message: `Não há stock suficiente para o produto ${vendaRequest.referencia}.` });
                }
                ...
                if (produtoEncontrado && produtoEncontrado.preco && vendaRequest.quantity) {
                    ...
                    return res.status(400).json({ message: `Dados de entrada inválidos para calcular o total da venda.` });
                }
                ...
                console.error(error);
                res.status(500).json({ message: "Erro ao processar a venda. Por favor, tente novamente." });
            }
        })
        .put(Utilizadores.authorize([scopes["administrador"], scopes["utilizador"], scopes["gestor"]]), verifyToken, async function (req, res, next) {
            try {
                ...
                if (!vendaExistente) {
                    return res.status(404).json({ message: `Venda com o número ${nrVenda} não encontrada.` });
                }
                ...
                for (let produto of produtos) {
                    ...
                    if (!produtoStock) {
                        return res.status(400).json({ message: `Não existe stock para o produto com a referência ${produto.refProduto}.` });
                    }

                    const produtoEncontrado = await Produto.findByRefProduto(produto.refProduto);
                    if (!produtoEncontrado) {
                        return res.status(400).json({ message: `Produto não encontrado com a referência ${produto.refProduto}.` });
                    }

                    // Verificar se há stock suficiente
                    if (produtoStock.quantidade < produto.quantity) {
                        return res.status(400).json({ message: `Não há stock suficiente para o produto com a referência ${produto.refProduto}.` });
                    }
                ...
                console.error(error);
                res.status(500).json({ message: "Erro ao atualizar a venda. Por favor, tente novamente." });
            }
        });
    ...
    router.route("/venda/finalizar")
        .get(Utilizadores.authorize([scopes["utilizador"], scopes["administrador"]]), verifyToken, function (req, res, next) {

            let usernameUtilizador = req.username;
            Vendas.findFinalizada(usernameUtilizador)
                .then((venda) => {
                    res.status(200).send(venda);
                })
                .catch((err) => {
        ...
        .put(Utilizadores.authorize([scopes["administrador"], scopes["utilizador"]]), verifyToken, async (req, res) => {
            try {
