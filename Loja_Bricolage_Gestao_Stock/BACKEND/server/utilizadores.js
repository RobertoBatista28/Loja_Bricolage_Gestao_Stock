...
const utilizadorRouter = () => {
    ...
    router.use(function (req, res, next) {
        ...
        if (req.originalUrl && (req.originalUrl.includes('/menu/produtos') && req.method === 'GET')) {
            next();
        } else if (req.originalUrl && (req.originalUrl.includes('/menu/produto/preco-maximo') && req.method === 'GET')) {
                next();
        } else {
    ...
    router.route("/utilizador/me")
        ...
        .put(verifyToken, async (req, res, next) => {
            try {
                ...
                if (body.username && body.username !== originalUsername) {
                    return res.status(200).send({ message: "Username atualizado, faça o login novamente", logout: true });
                }
        ...
        .delete(Utilizadores.authorize([scopes["administrador"], scopes["gestor"], scopes["utilizador"]]), verifyToken, function (req, res) {
            let username = req.username;
            Utilizadores.deleteByUsername(username)
                .then(() => { res.status(200).send("Utilizador removido com sucesso"); })
                .catch((err) => { res.status(500).json("Erro ao remover o utilizador", err); });
        })
