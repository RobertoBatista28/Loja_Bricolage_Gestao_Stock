...
const vendaController = require("../data/venda/vendaController");
...
const stockRouter = () => {
    ...
    router.route("/stock")
        ...
        .post(Utilizadores.authorize([scopes["administrador"], scopes["gestor"]]), verifyToken, async function (req, res, next) {
            let body = req.body;

            const referencia = await Stocks.gerarAutoIncremental();
                ...
                .then((produto) => {
                    if (produto) {
                        Stocks.findByReferencia(referencia)
                            .then((verificarref) => {
                                if (verificarref) {
                                    res.status(400).send("Essa referência de Stock já está em uso, escolha outra!");
                                } else {
                                    Stocks.findByRefProduto(body.refProduto)
                                        .then((verificarStock) => {
                                            if (verificarStock) {
                                                ...
                                                body.anotacoes = body.anotacoes || "";
                                                body.data = new Date();
                                                Stocks.create(body)
                                                    .then(() => { res.status(200).send("Stock adicionado com sucesso."); })
                                                    .catch((err) => { res.status(500).send("Erro ao adicionar stock!", err); });
                                            }
                                        })
    ...
    .put(Utilizadores.authorize([scopes["administrador"], scopes["gestor"]]), verifyToken, function (req, res, next) {
        ...
        if (typeof quantity !== 'number' || quantity < 0) {
            res.status(400).send("Valor de quantidade inválido.");
   ...
   router.route("/stocks/criar").post(Utilizadores.authorize([scopes["administrador"], scopes["gestor"]]), verifyToken, function (req, res, next) {
