...
function utilizadorController(UtilizadorModel) {
    ...
    async function findByVerificationToken(token) {
        try {
            return await UtilizadorModel.findOne({ verificationToken: token });
        } catch (error) {
            console.error("Erro ao encontrar utilizador pelo token de verificação:", error);
            throw error;
        }
    }

    function updateVerified(username, utilizador) {
        return new Promise(function (resolve, reject) {
        ...
        return new Promise(function (resolve, reject) {
            UtilizadorModel.findOne({ username })
                .then((user) => {
                    ...
                    if (add) {
                        if (index === -1) {
                            user.favoritos.push(referencia); 
                        }
                    } else {
                        if (index !== -1) {
