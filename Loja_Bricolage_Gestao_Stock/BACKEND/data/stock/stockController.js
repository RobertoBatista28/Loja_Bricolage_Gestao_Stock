...
function stockController(StockModel) {
    ...
    function gerarAutoIncremental() {
        return new Promise(function (resolve, reject) {
            StockModel.find({}).sort({ referencia: 1 })
                .then((stock) => {
                    if (stock.length === 0) {
                        resolve(1); // Se não houver vendas, retorna 1
                    } else {
                        let ultimoReferencia = stock[stock.length - 1].referencia;
                        resolve(ultimoReferencia + 1); // Retorna o próximo número de venda disponível
                    }
                })
                .catch((err) => reject(err));
        });
